...
def parse_args():
    ...
    p.add_argument("--data-source", choices=["mt5","csv","ctrader","fix"]; p.add_argument("--timeframe", choices=["M1","M5","M15","H1"], default="M1")
    ...
    # cTrader (all passed at runtime; no env)
...
def get_data_provider(args):
    if args.data_source == "ctrader":
        ...
        required = [
            args.ctrader_client_id, args.ctrader_client_secret,
            args.ctrader_access_token, args.ctrader_account_id
        ]
        if not all(required):
