...
def parse_signals_from_messages(messages: List[Dict]) -> List[Signal]:
    out = []
    for m in messages:
        text = m["text"]; dt = m["date"]
        if dt.tzinfo is None: dt = dt.replace(tzinfo=timezone.utc)
        mo = SIG_RE.search(text.replace("\\n"," "))
        if not mo: continue
        side = mo.group("side").upper(); symbol = mo.group("symbol").upper()
        entry = float(mo.group("entry")); sl = float(mo.group("sl"))
        tps = [float(x[0]) for x in TP_RE.findall(mo.group("tps"))]
        if not tps: continue
        out.append(Signal(dt=dt, side=side, symbol=symbol, entry=entry, sl=sl, tps=tps, raw_text=text))
    return out
